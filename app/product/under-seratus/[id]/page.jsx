...
import CarouselDetailProduct from "@/components/carousel/CarouselDetailProduct";
...
const ProductPage = ({params}) => {
  ...
  const getProduct = async () => {
    try {
      const { products } = await getProdukUnderSeratus();
      console.log(products);
      const data = products.find((product) => product.product_id === params.id);
      console.log(data);
      setDetail(data);
    } catch (error) {
      console.error("ERROR PRODUK UNDER SERATUS:", error);
    }
  };
          ...
          <Box bg={"white"} p={4} borderRadius={"lg"} mt={4}>
            ...
            <Flex justifyContent={"space-between"} alignItems={"center"} mt={2}>
              ...
              <Stack fontSize={"sm"}>
                ...
                <Button  h={6} fontSize={"sm"} bg={"rgb(255, 214, 0)"}>Add To Cart</Button>
              </Stack>
            </Flex>
          </Box>
        </Box>
